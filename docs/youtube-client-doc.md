...
| `channelBanner` | No | Yes | No | No |
| `channelSection` | Yes | Yes | Yes | Yes |
| `comment` | Yes | Yes | Yes | Yes |
| `commentThread` | Yes | Yes | Yes | No |
| `guideCategory` | No | No | No | No |
| `i18nLanguage` | Yes | No | No | No |
| `i18nRegion` | Yes | No | No | No |
| `playlist` | Yes | Yes | Yes | Yes |
| `playlistItem` | Yes | Yes | Yes | Yes |
| `search result` | Yes | No | No | No |
| `subscription` | Yes | No | No | No |
| `thumbnail` | No | No | No | No |
...
- A [search query](https://developers.google.com/youtube/v3/docs/search/list) costs `1` unit.
- A [video upload](https://developers.google.com/youtube/v3/docs/videos/insert) costs `1` unit.
...
- The `https://developers.google.com/youtube/v3/getting-started#part` parameter identifies groups of properties that should be returned for a resource.
- The `https://developers.google.com/youtube/v3/getting-started#fields` parameter filters the API response to only return specific properties within the requested resource parts.

<br />

### How to use the `part` parameter

The `part` parameter is a required parameter for any API request that retrieves or returns a resource. The parameter identifies one or more top-level (non-nested) resource properties that should be included in an API response. For example, a `https://developers.google.com/youtube/v3/docs/videos#resource` resource has the following parts:
